...
(function(){
  ...
  const KINGKILLERWIKIA_REGEX = /^(kkc|kingkiller)\.(wikia|fandom)\.com$/i; // Used to match the domain of the old wikia/fandom to make sure we are redirecting the correct domain.
  ...
  chrome.webNavigation.onBeforeNavigate.addListener(
    function(info) {
      if(isPluginDisabled) { // Ignore all navigation requests when the extension is disabled.
        console.log("KingkillerWikia intercepted, ignoring because plugin is disabled.");
        return;
      }
      ...
      const isWikia = KINGKILLERWIKIA_REGEX.test(url.host); // Check to ensure the redirect is occurring on either the fandom/wikia domain.
      // If domain isn't subdomain of wikia.com, ignore, also if it's not in the redirect filter
      if (!isWikia) return;

      // Generate new url
      const host = 'kingkiller';
      const redirectUrl = `https://${host}.wiki${url.pathname.replace(/^\/wiki\//i,"/w/")}`; // Create the redirect URL
      console.log(`KingkillerWikia intercepted:  ${info.url}\nRedirecting to ${redirectUrl}`); 
  ...
  storage.onChanged.addListener(
      function(changes, areaName) {
        ...
        if(changes["isDisabled"]!==undefined && changes["isDisabled"].newValue!=changes["isDisabled"].oldValue) {
          console.log(`Kingkiller Wiki Redirector is now ${changes["isDisabled"].newValue?'disabled':'enabled'}`);
